...
// Kilter app report an integer "difficulty" that indexes this table; it is static shared data, so
// it's bundled here rather than synced. Neither board reliably returns it over its sync API.
//
// Compact source rows of [id, "font/V", french, yds]. Font and V-scale are split from the label.

export interface DifficultyGrade {
  /** Compound label as the apps show it, e.g. "7C/V9". */
  label: string;
...
// difficulty id -> the grade on every scale, so callers can pick Font, V, French, or YDS.
export const DIFFICULTY_GRADES: Readonly<Record<number, DifficultyGrade>> = Object.fromEntries(
  ROWS.map(([id, label, french, yds]) => {
    const [font, vScale] = label.split("/") as [string, string];
    return [id, { label, font, vScale, vGrade: parseVGrade(vScale) ?? 0, french, yds }];
  }),
);

// Resolves an integer difficulty to its grade on every scale, or undefined if unknown. Aurora
// consensus difficulties can be fractional, so the id is rounded first.
export function gradeForDifficulty(difficulty: number | null | undefined): DifficultyGrade | undefined {
  return difficulty == null ? undefined : DIFFICULTY_GRADES[Math.round(difficulty)];
}
